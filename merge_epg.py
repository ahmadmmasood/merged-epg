...
# ---------------- Update index.html ----------------
def update_index_page(channels_count, programs_count, file_size_mb):
    ...
    # Replace the placeholders in the index.html with the new data
    updated_content = content.replace(
        "<strong>Last updated:</strong> 2026-02-13 17:28:11 EST",
        f"<strong>Last updated:</strong> {timestamp}"
    )
    updated_content = updated_content.replace(
        "<strong>Channels kept:</strong> 0",
        f"<strong>Channels kept:</strong> {channels_count}"
    )
    updated_content = updated_content.replace(
        "<strong>Programs kept:</strong> 0",
        f"<strong>Programs kept:</strong> {programs_count}"
    )
...
# Call function to update index
update_index_page(channels_count, programs_count, file_size_mb)
